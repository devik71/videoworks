...
# Whisper зрідка віддає слова з нульовою тривалістю (на довгому матеріалі це
# кілька відсотків), і репліка, зібрана лише з них, виходить невидимою.
# Показати її з накладанням на сусідню — менше зло, ніж не показати взагалі.
MIN_VISIBLE = 0.2
...
def _fix_timing(cues: list[Cue], opts: CueOptions) -> None:
    ...
    for index, cue in enumerate(cues):
        ...
        if cue.end > limit:
            cue.end = max(cue.start + MIN_VISIBLE, limit)

        # Остання лінія оборони. Затиснення до наступної репліки могло звести
        # тривалість у нуль — а нульова репліка просто не зʼявиться на екрані.
        cue.end = round(max(cue.end, cue.start + MIN_VISIBLE), 3)
